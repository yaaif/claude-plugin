...
4. For multi-capability use cases (chat + ambient + desktop, or unclear split), start with `yaaif-plan-usecase`: write a plan, wait for approval, then create a Scenario (`yaaif_agent_spec_create`) and execute. Do not jump straight to a single create skill. To create or maintain a Scenario (Agent Spec) — including Admin UI **Open in Claude Code** with a `spec_id` — use `/yaaif-platform:yaaif-scenario` and `yaaif_agent_spec_*`.
